...
func SmbScanConn(host string) bool {
	res, _, err := smb2(host, 445, 2*time.Second)
...
func SmbCheck(hostlists []string, threadNum int, logfile string) {
	...
	for i := 0; i < threadNum; i++ {
		go SmbScan(chanHosts, wg, logfile)
	}
...
func SmbScan(chanhost chan string, wg *sync.WaitGroup, logfile string) {
	for host := range chanhost {
		if SmbScanConn(host) {
			utils.LogSuccess(host, "smb", logfile)
			wg.Done()
		} else {
			wg.Done()
		}
